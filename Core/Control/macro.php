...
Form::macro('ctlTexter', function($name, $label = null){
...
Form::macro ( 'ngSelectField', function ($name, $label = null, $ngModel, $ngOptions, $attributes = array()) {
	$attributes = array_merge(array( 'ng-model' => $ngModel ), $attributes );
	$attributes = array_merge(array( 'ng-options' => $ngOptions ), $attributes );
	$element = Form::select ( $name, array(), null, fieldAttributes ( $name, $attributes ) );
	return fieldWrapper ( $name, $element, $label );
});
